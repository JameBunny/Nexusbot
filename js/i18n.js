...
const translations = {
    ...
    th: {
        ...
        terms: {
            ...
            section4Content:  "บอทให้บริการ \"ตามสภาพ\" (as is) เราไม่รับประกันว่าบอทจะทำงานได้ตลอดเวลาโดยไม่มีข้อผิดพลาด",
    ...
    ja: {
        ...
        terms: {
            ...
            section4Content:  "Botは「現状のまま」で提供されます。Botが常時エラーなく動作することを保証しません。",
...
function applyTranslations(lang) {
    ...
    for (var i = 0; i < elements.length; i++) {
        ...
        var translation = getNestedValue(langData, key);
    ...
    for (var j = 0; j < placeholderElements.length; j++) {
        ...
        var pTranslation = getNestedValue(langData, pKey);
        if (pTranslation !== undefined) {
            el.placeholder = pTranslation;
        }
    }
    ...
    if (lang === 'ja') {
        ...
        document.body.style.fontFamily = "'Inter', 'Noto Sans Thai', 'Noto Sans JP', sans-serif";
    }
...
function changeLanguage(lang) {
    ...
    var currentLangElement = document.getElementById('currentLang');
    if (currentLangElement) {
        if (lang === 'th') {
            currentLangElement.textContent = 'TH';
        } else if (lang === 'en') {
            currentLangElement.textContent = 'EN';
        } else if (lang === 'ja') {
            currentLangElement. textContent = 'JP';
        }
    }
...
function getCurrentLanguage() {
    try {
        var saved = localStorage.getItem('nexus-language');
